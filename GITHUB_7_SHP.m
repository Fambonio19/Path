...
start_lat = 40.9189447975; % Starting latitude
start_lon = 9.5774832291; % Starting longitude
...
function [route, total_time] = astar(speed_grid, start_point, end_point)
    ...
    while ~isempty(pq)
        ...
        for d = 1:size(directions, 1)
            ...
            if new_point(1) > 0 && new_point(1) <= rows && new_point(2) > 0 && new_point(2) <= cols && ~isnan(speed_grid(new_point(1), new_point(2)))
...
for i = 1:length(time_stamps)
    ...
    for r = 1:rows
        for c = 1:cols
            if isnan(wave_height(r, c)) || isnan(wave_direction(r, c)) % Check for no data values
                speed_grid(r, c) = NaN;
            else
